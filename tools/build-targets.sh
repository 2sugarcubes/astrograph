...
for target in {{x86_64,i686}-{unknown-linux,pc-windows}-gnu,arm-unknown-linux-gnueabihf}; do
  ...
  cross build --bin astrograph --release --target "${target}" >/dev/null 2>&1 && echo "${target} ✅" || echo "${target} ❌"
done
...
move_bin() {
  mv target/$1/release/astrograph release/astrograph-$2
}
...
mv target/x86_64-unknown-linux-gnu/release/build/astrograph-bin-*/out/* release/completions
mv release/completions/astrograph.1 release/manpage
